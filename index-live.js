/**
 * Polymarket Arb Bot — BTC, ETH, SOL, XRP  |  5-min + 15-min windows
 *
 * Strategies:
 *   1. BOTH-SIDES ARB  — YES + NO combined < 0.95 → guaranteed profit
 *   2. LATE ENTRY MOMENTUM (LEM) — final window entry on Binance delta ≥ 0.2%
 *   3. CROSS-ASSET CORRELATION   — strong BTC signal pre-enters ETH/SOL/XRP
 *
 * Run:  npm run live   (SIM mode by default — set LIVE_MODE=true for real orders)
 */
...
// Price display auto-scaled per asset
const fmtPx = (p, asset) => {
  if (p == null) return "...";
...
// ── Binance trade feed ────────────────────────────────────────────────────────
// Tracks real-time price AND buy/sell volume pressure via the trade stream.
// m:false = aggressive buy (taker bought), m:true = aggressive sell (taker sold).
function startPriceFeed(symbol) {
  ...
  // Volume in 1-second buckets — keeps last 2 minutes of history
  const volBuckets = [];
      ...
      try {
        ...
        if (Number.isFinite(qty)) {
          ...
          if (bts !== curBkt.ts) {
            ...
            if (volBuckets.length > 120) volBuckets.shift(); // 2-min cap
            curBkt = { buyV: 0, sellV: 0, ts: bts };
          }
          if (!t.m) curBkt.buyV  += qty; // buyer is taker = aggressive buy
          else      curBkt.sellV += qty; // seller is taker = aggressive sell
        }
      } catch { /* ignore */ }
    });
    const retry = () => {
  ...
  return {
    get: () => price,
    // Returns buy ratio 0–1 over the last windowMs (default 60s). 0.5 = neutral.
...
function render({
  ...
  out.push(hdr(`Polymarket Arb — BTC ETH SOL XRP  ${mode}`));
  ...
  // Asset prices
  const pxStr = Object.entries(feedPrices)
    .map(([a, p]) => `${a} ${fmtPx(p, a)}`).join("  ");
  out.push(row(`${C.dim}${pxStr}${C.reset}`));
  out.push(row(""));

  // Momentum + 15m trend + volume pressure — all assets on one line
  ...
  // ── Sweep follow section ─────────────────────────────────────────────────────
  out.push(row(""));
  out.push(sec("SWEEP FOLLOW  (order-book momentum)"));
  out.push(row(`Followed: ${C.bgreen}${sweepStats.followed}${C.reset}  ${C.dim}(fires when ask price rises ≥1.2¢ in 5s)${C.reset}`));
  ...
  if (copyTrader?.wallet) {
    out.push(row(""));
    out.push(sec("COPY TRADER  (following bonereaper)"));
    const pollAge = copyTrader.lastPoll ? `${Math.round((now - copyTrader.lastPoll) / 1000)}s ago` : "waiting";
    const ctStatus = copyTrader.error
      ? `${C.red}● ERR: ${copyTrader.error}${C.reset}`
...
async function main() {
  ...
  // ── Binance price feeds — one WebSocket per asset ──────────────────────────
  const feeds = {};
  for (const [asset, symbol] of Object.entries(CONFIG.binanceSymbols)) {
    feeds[asset] = startPriceFeed(symbol);
  }

  // Price snapshots every 30s — 6 samples = ~3 min of momentum history
  ...
  const snapPrices = () => {
    for (const [asset, feed] of Object.entries(feeds)) {
      ...
      if (p) {
        ...
        lateEntry.recordPriceSnap(asset, p); // feeds 15m/1h trend history
      }
    }
  };
  ...
  // Arb sizing: Kelly formula for both-sides positions
  ...
  // Directional sizing: half-Kelly scaled by signal confidence.
  // fullKelly = (p×(1+b) − 1) / b  where b = payout odds, p = estimated win rate
  const kellyBet = (entryPrice, confidence) => {
    const wr     = CONFIG.estimatedWinRate;
    const b      = (1 - entryPrice) / entryPrice; // profit per dollar risked
  ...
  clobWs.onOpportunity((marketId, yesPrice, noPrice) => {
    ...
    (async () => {
      try {
        const betSize = kellySizeBet(yesPrice + noPrice);
  ...
  // ── Sweep follow (reacts to order-book sweeps in milliseconds) ─────────────
  ...
  // When a strong LEM signal fires on one asset, enter correlated markets too.
  // BTC leads all crypto; confidence is discounted 25% for cross-asset entries.
    ...
    for (const market of marketList) {
      ...
      (async () => {
        try {
          const allocated = [...activePositions.values()].reduce((s, p) => s + (p.totalSpent ?? 0), 0);
          const available = Math.max(0, simBalance - allocated);
          const betSize   = Math.min(CONFIG.maxTradeUsdc * crossConf, available * 0.15);
          if (betSize < 1) return;
          const pos = new DirectionalPosition({
            id: market.id, asset: market.asset,
  ...
  // ── Late Entry Momentum check (every 2s) ────────────────────────────────────
  // 5-min: enter 15–90s before close  |  15-min: enter 45–270s before close
        ...
        (async () => {
          try {
            ...
            if (entered) {
              ...
              activePositions.set(market.id, pos);
              lemStats.entered++;
              // Strong BTC/ETH move → pre-enter correlated assets immediately
  ...
  // ── Monitor open positions (every 3s) ───────────────────────────────────────
  const monitor = async () => {
    ...
    try {
      for (const [id, pos] of activePositions) {
        if (pos.type === "directional") {
          // Early profit lock: close deeply in-the-money positions before reversal risk
  ...
  // ── Copy trader (optional — set COPY_WALLET=0x... in .env) ─────────────────
  ...
  process.on("SIGINT", async () => {
    ...
    process.stdout.write(
      `\n\nStopped.\n` +
      `ARB: entered=${stats.entered}  both-filled=${stats.bothFilled}  P&L=+${fmtUsd(stats.guaranteedProfit)}\n` +
